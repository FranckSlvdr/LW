...
export function ImportUploader({ weeks }: ImportUploaderProps) {
  ...
  const t = getImportsMessages(locale).uploader
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error?.message ?? t.errors.import)
      }

      const result = data.data
      setStatus('success')
      setMessage(t.success(result.rowsImported, result.rowsSkipped))
      setFile(null)
    ...
    <Card>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 mt-2">
        <div className="flex gap-3">
          {(['scores', 'players'] as const).map((type) => (
            <label
              key={type}
              className={[
            ...
            >
              <input
                ...
                className="sr-only"
              />
              <span>{type === 'scores' ? '\u{1F4CA}' : '\u{1F465}'}</span>
              <span className="font-medium">
                {type === 'scores' ? t.importTypes.scores : t.importTypes.players}
              </span>
            </label>
          ))}
        </div>
        ...
        {importType === 'scores' && (
          <div>
            <label className="label-xs block mb-1">{t.week}</label>
            <select
              value={weekId}
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-raised)] w-fit">
          ...
          <span className="text-xs text-[var(--color-text-muted)]">{t.template}</span>
          <a
            href={importType === 'players' ? '/templates/joueurs-template.csv' : '/templates/scores-vs-template.csv'}
            download
        ...
        <div>
          <label className="label-xs block mb-1">{t.file}</label>
          <input
            ref={fileRef}
            type="file"
            accept=".csv,text/csv"
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {status === 'loading' ? t.importing : t.submit}
          </button>
          {message && (
